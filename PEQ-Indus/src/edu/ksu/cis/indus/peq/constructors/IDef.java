 ...
 */
public class IDef extends GeneralConstructor {
    ...
    public IFSMToken match(GeneralConstructor cons, final Edge masterEdge) throws IllegalAccessException {
        ...
        if (cons instanceof IDef) {
            if (this.isVariablePresent() && cons.isVariablePresent()) {
                throw new IllegalAccessException("Both the constructors can't have labels");
            }            
            final Pair _pair = (Pair) ((Node) masterEdge.getDstnNode()).getInformation();
            final DefinitionStmt _dsdt = (DefinitionStmt) _pair.getFirst();
    ...
    public boolean equals(Object object) {
        if (!(object instanceof IDef)) {
            return false;
        }
        IDef rhs = (IDef) object;
     ...
     */
    public int hashCode() {
        return new HashCodeBuilder(1922390563, -76954693).appendSuper(
