...
public class KaveriErrorLog {
     ...
     * @param exception
     */
    private static void addToLog(int error, int errorCode, String message, Throwable exception) {
       final ILog _log = KaveriPlugin.getDefault().getLog();
       final String _pluginid = KaveriPlugin.getDefault().getBundle().getSymbolicName();
       final IStatus _status = new Status(error, _pluginid, errorCode, message, exception);
       _log.log(_status); 
    }
}
