			...
			try {
			    ...
			    if (_retObj instanceof IMethod) {			    
					...
					final ScopePropertiesSelectionDialog _spsd = new ScopePropertiesSelectionDialog(
					        Display.getCurrent().getActiveShell(), IScopeDialogMorphConstants.SCOPE_NAME_ONLY, "Method_" + 
			    ...
			    if  (_retObj instanceof IField) {
			        ...
			        final ScopePropertiesSelectionDialog _spsd = new ScopePropertiesSelectionDialog(
					        Display.getCurrent().getActiveShell(), IScopeDialogMorphConstants.SCOPE_NAME_ONLY, "Field_" + 
					        _field.getElementName());
			        if (_spsd.open() == IDialogConstants.OK_ID) {					    
			    ...
			    if (_retObj instanceof IType) {
			        ...
			        final ScopePropertiesSelectionDialog _spsd = new ScopePropertiesSelectionDialog(
					        Display.getCurrent().getActiveShell(), IScopeDialogMorphConstants.SCOPE_NAME_REGEX, "Class_" + 
					        _type.getElementName());
			        _spsd.setStrDefaultClassName(_type.getFullyQualifiedName());
			        if (_spsd.open() == IDialogConstants.OK_ID) {
			            final ClassSpecification _cs = getClassSpecification(_type, _spsd.getStrScopeName(),_spsd.getStrChoice(), _spsd.getStrClassRegex());
			            if (_cs != null) {
			    ...
			    setValue(scopePreferenceId, _scopeSpec);
