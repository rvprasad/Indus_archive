...
/**
 * @author ganeshan Displayes the scope selection dialog. The set of enabled scope elements is maintained by the plugin
 *         preference. The purpose of this dialog is to enable the user to pick the relevant scope elements for an instance of
 *         the slice.
 */
public class ScopeDialog
	...
	/*
	 * _comp.setLayout(new GridLayout(1, true)); final Group _grp = new Group(_comp, SWT.BORDER); _grp.setText("Pick the
	 * elements to be included in the scope"); _grp.setLayout(new GridLayout(1, true)); GridData _gd = new GridData();
	 * _gd.grabExcessHorizontalSpace = true; _gd.horizontalSpan = 1; _gd.horizontalAlignment = GridData.FILL;
	 * _gd.grabExcessVerticalSpace = true; _gd.verticalAlignment = GridData.FILL; _gd.widthHint =
	 * IDialogConstants.MINIMUM_MESSAGE_AREA_WIDTH; _gd.heightHint = IDialogConstants.MINIMUM_MESSAGE_AREA_WIDTH * 3 / 4;
	 * _grp.setLayoutData(_gd); final Table _table = new Table(_grp, SWT.SINGLE | SWT.V_SCROLL | SWT.CHECK);
	 * _table.setLinesVisible(true); _table.setHeaderVisible(true); _gd = new GridData(); _gd.horizontalSpan =1;
	 * _gd.grabExcessHorizontalSpace = true; _gd.grabExcessVerticalSpace = true; _gd.horizontalAlignment = GridData.FILL;
	 * _gd.verticalAlignment = GridData.FILL; _table.setLayoutData(_gd); final TableColumn _col1 = new TableColumn(_table,
	 * SWT.NONE); _col1.setText("!"); final TableColumn _col2 = new TableColumn(_table, SWT.NONE); _col2.setText("Type");
	 * final TableColumn _col3 = new TableColumn(_table, SWT.NONE); _col3.setText("Scope Name"); final TableColumn _col4 = new
	 * TableColumn(_table, SWT.NONE); _col4.setText("Element Name"); final TableItem _tt = new TableItem(_table, SWT.NONE);
	 * _tt.setText(new String[]{"", "Class", "myScope", "myPackage.Readers"}); final TableColumn _tc[] = _table.getColumns();
	 * for (int i = 0; i < _tc.length; i++) { _tc[i].pack(); } _comp.addControlListener( new ControlAdapter() { public void
	 * controlResized(ControlEvent e) { final TableColumn _cols[] = _table.getColumns(); for (int i = 0; i < _cols.length;
	 * i++) { _cols[i].pack(); } } } );
	 */
