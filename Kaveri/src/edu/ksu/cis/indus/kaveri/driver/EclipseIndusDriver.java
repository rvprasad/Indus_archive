...
public class EclipseIndusDriver
  extends SootBasedDriver {
	 ...
	 * @param classes The set of class names to set to
	 * application classes
	 */
	...
	/**
	 * This function is used to extract line numbers of Java statements from the corresponding Jimple statements which have a
	 * slice tag assiciated with them. Returns a mapping of class names with their associated line numbers.
	 * <b>Postcondition:</b>  result.oclIsKindOf(HashMap) && result.keys.oclIsKindOf(String : classname) &&
	 * result.values.oclIsKindOf(Map) and result.values.values.oclIsKindOf(Collection(AnnotationData))
	 *
			...
			for (int _nCtr = 0; _nCtr < _list.size(); _nCtr++) {
				...
				/*                final int _line = getLineNumberFromMethod(_method);
				   if (_line > -1) {
				       ...
				       _data.setComplete(true);
				       _data.setNLineNumber(_line);
				       if (!_lst.contains(_data)) {
				           _lst.add(_data);
				       }
				   }
				 */
	 ...
	 *
	 * @param unit The Jimple Statement from which the line number is to be extracted Postcondition: result == -1 if no
	 * 		  useful tags present, result = linenumber otherwise
	 *
	 ...
	 */
	private void processUnit(final Chain unitchain, final List lst, final String classname, final String methodname) {
