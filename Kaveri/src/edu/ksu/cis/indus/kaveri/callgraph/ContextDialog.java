...
public class ContextDialog extends Dialog {
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        viewer = CheckboxTableViewer.newCheckList(_grp, SWT.SINGLE | SWT.FULL_SELECTION);
        ...
        for (int i = 0; i < _colNames.length; i++) {
            ...
            _col.setText(_colNames[i]);
        }
        viewer.setContentProvider(new ContextContentProvider());
        viewer.setLabelProvider(new ContextLabelProvider());
        viewer.setInput(KaveriPlugin.getDefault().getIndusConfiguration().getCtxRepository());
        for (int i = 0; i < _colNames.length; i++) {
