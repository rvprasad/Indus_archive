...
public class AbstractAutomata
  implements IAutomata {
	...
	public Object clone() {
		try {
			return (AbstractAutomata) super.clone();
	...
	public void performTransitionOn(final ITransitionLabel label) {
		final Collection _dests = sng.getDestOfOutgoingEdgesLabelled(sng.queryNode(currentState), label);
	 ...
	 * @return DOCUMENT ME!
	 */
	protected Collection getResultingStates(final IState state, final ITransitionLabel label) {
		return CollectionUtils.collect(sng.getDestOfOutgoingEdgesLabelled(sng.queryNode(state), label),
