...
public class IndusConfigurationDialog
  extends Dialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		initializeList(criteriaList, project);
		...
		private void handleDelete(final Button btnDelete, final IJavaProject theproject) {
			btnDelete.addSelectionListener(new SelectionAdapter() {
				...
				private void removeSelection(final String selectedString,
					final IJavaProject theproject, final int index) {
				IResource _resource;
				try {
					...
					try {
						...
						if (_propVal != null) {
							...
							_xstream
									.alias(
											Messages
													.getString("IndusConfigurationDialog.8"), CriteriaData.class); 
		 ...
		 */
		private void initializeList(final List criteriasList, final IJavaProject javaproject) {
			IResource _resource;
	...
	protected void okPressed() {
		...
		if (_selectedConfiguration.equals(Messages.getString("IndusConfigurationDialog.25"))) { //$NON-NLS-1$
			_configString = SliceEclipsePlugin.getDefault().getPreferenceStore()
					.getString(Messages.getString("IndusConfigurationDialog.26")); //$NON-NLS-1$
		} else {
			final XStream _xstream = new XStream();
		...
		SliceEclipsePlugin.getDefault().getIndusConfiguration()
				.setCurrentConfiguration(_configString);
		setUpCriteria();
		super.okPressed();
	}
	...
	private void setUpCriteria() {
		final int[] _currentsels = criteriaList.getSelectionIndices();
		SliceEclipsePlugin.getDefault().getIndusConfiguration().getCriteria()
				.clear();
